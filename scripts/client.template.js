...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const COMPOSER_CSS = [
      ...
      //
      // The seat's own backdrop fades into --dsw-alias-bg-base, which this theme
      // makes SEMI-transparent, so scrolled messages could still ghost through.
      // Frost it with a backdrop blur so content behind the composer is not
      // legible (sticky-composer-over-scrolling-content, the standard cure).
      '.wSkVaW_root[data-phase=active] .wSkVaW_composerSeat { backdrop-filter: blur(10px) saturate(130%); -webkit-backdrop-filter: blur(10px) saturate(130%); }',
    ].join('\n')
